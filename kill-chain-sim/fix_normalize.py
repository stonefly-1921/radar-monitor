...
old_func = '''    def normalize_entity_id(self, entity_id: EntityId, entity_type=None) -> EntityId:
        ...
        The raw entity_id.entity field (36864) is a stable identifier but
        does not match AFSIM's internal DIS entity ID.
        ...
        """
        key = str(entity_id.entity_id)  # Use entity field as stable key
        with self._lock:
            if key not in self._entity_id_map:
                # Use specific field from entity_type if available
                if entity_type is not None and entity_type.specific > 0:
                    # specific field directly indicates AFSIM's entity number
                    normalized_entity = entity_type.specific
                    logger.debug(f"Entity ID mapping: {entity_id} -> 25:1:{normalized_entity} (via specific={entity_type.specific})")
                else:
            ...
            original_entity = self._entity_id_map[key]
            return EntityId(site_id=25, application_id=1, entity_id=original_entity)'''
...
if old_func in content:
    ...
    # Find the function
    import re
    m = re.search(r'def normalize_entity_id\(self.*?return EntityId\(site_id=25.*?entity_id=original_entity\)', content, re.DOTALL)
    if m:
