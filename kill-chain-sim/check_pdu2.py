...
print(f'Total length: {len(pdu_bytes)} bytes')
print(f'Expected: 12 (header) + 84 (fire data) = 96 bytes')
...
print(f'  Length (2 bytes): {pdu_bytes[8:10].hex()} = {int.from_bytes(pdu_bytes[8:10], "big")} bytes')
print(f'  Padding (2 bytes): {pdu_bytes[10:12].hex()}')
print(f'  Fire Data ({len(pdu_bytes)-12} bytes): {pdu_bytes[12:].hex()}')