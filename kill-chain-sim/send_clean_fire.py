...
# Body: 84 bytes (fire mission data)
...
timestamp_bytes = struct.pack('>I', 0)  # timestamp = 0
header_fields = struct.pack('>BBBBHH', 6, 1, 2, 1, 0, 0)  # version=6, exercise=1, type=2, family=1, length=0, padding=0

# Set actual length
total_len = 12 + len(body)
header = timestamp_bytes + header_fields[:8] + struct.pack('>H', total_len) + header_fields[10:]
