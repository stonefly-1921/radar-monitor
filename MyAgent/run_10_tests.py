...
3. 用 python_run 计算:从北京(经度116.4*E,纬度39.9*N)到台北(经度121.5*E,纬度25*N)的理论弹道,假设初速度 3000m/s,射角 45 degrees
...
for i, task_info in enumerate(TASKS):
    ...
    if ok:
        ...
        results.append(f'[OK] 任务{i+1}: {task_info["name"]}')

    else:

        failed += 1

        results.append(f'[X] 任务{i+1}: {task_info["name"]}')
